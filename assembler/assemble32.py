...
codes = {
	...
	"ldw":0x4002, "stw":0x5002, "bez":0xc000, "bnz":0xd000, "hcf":0x0003,
	"ldc0":0x9000, "ldc1":0x9000, "ldc2": 0x9000, "ldc3": 0x9000
}
			...
			if len(flds) > 1 :
				...
				if flds[0] == "ldb" and len(parts) == 2 :
					if lookup.get(parts[1]) == None :
						...
						program.insert(i+1, "\tldc1	sr," + parts[1] + "\n")
						program.insert(i+2, "\tldc2	sr," + parts[1] + "\n")
						program.insert(i+3, "\tldc3	sr," + parts[1] + "\n")
						program.insert(i+4, "\tldb	" + parts[0] + ",r0,sr\n")
					else :
				...
				if flds[0] == "stb" and len(parts) == 2 :
					if lookup.get(parts[1]) == None :
						...
						program.insert(i+2, "\tldc2	sr," + parts[1] + "\n")
						program.insert(i+3, "\tldc3	sr," + parts[1] + "\n")
						program.insert(i+4, "\tstb	r0," + parts[0] + ",sr\n")
					else :
				...
				if flds[0] == "ldw" and len(parts) == 2 :
					if lookup.get(parts[1]) == None :
						...
						program.insert(i+2, "\tldc2	sr," + parts[1] + "\n")
						program.insert(i+3, "\tldc3	sr," + parts[1] + "\n")
						program.insert(i+4, "\tldw	" + parts[0] + ",r0,sr\n")
					else :
				...
				if flds[0] == "stw" and len(parts) == 2 :
					if lookup.get(parts[1]) == None :
						...
						program.insert(i+2, "\tldc2	sr," + parts[1] + "\n")
						program.insert(i+3, "\tldc3	sr," + parts[1] + "\n")
						program.insert(i+4, "\tstw	r0," + parts[0] + ",sr\n")
					else :
...
def assemble(flds) :
	...
	if symb != None :
		...
		if len(parts) == 2 :
			...
			if (flds[0] == "ldc0") :				# ldc0 .. ldc3 are pseudo-ops
				return (opval | 0x0800 | (getval(parts[1]) << 8) | ((getval(parts[2]) >> 24) & 0xff))
			elif (flds[0] == "ldc1") :
				return (opval | 0x0800 | (getval(parts[1]) << 8) | ((getval(parts[2]) >> 16) & 0xff))
			elif (flds[0] == "ldc2") :
			...
			if not flds : print (lin),				# print now if only a symbol
				...
				try :
					...
					if flds2[0] == '"' and flds2[-1] == '"' :
						...
						print ("%08x %s    %s" % (pc, tohex(instruction), lin)),
						pc = pc + 2
						flds3 = flds3[1:]
						for f in flds3 :
							instruction = assemble(flds3)
						...
						if codes.get(flds[0]) == None :
							if (pc % 4 != 0) :	# align to word boundry (32 bits)
								...
								pc = pc + 2
							data = assemble(flds)
							print ("%08x %s %s" % (pc, tohex(data >> 16), lin)),
							print ("%08x %s" % (pc+2, tohex(data & 0xffff))),
							pc = pc + 4
							flds = flds[1:]
							for f in flds :
								data = assemble(flds)
								print ("%08x %s" % (pc, tohex(data >> 16)))
								print ("%08x %s" % (pc+2, tohex(data & 0xffff)))
								pc = pc + 4
								flds = flds[1:]
						else :
